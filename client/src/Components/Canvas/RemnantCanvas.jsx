...
function RemnantCanvas({ width = "100%", height = "500px" }) {
  ...
  const saveCanvas = async () => {
    const remnantCanvasData = await canvasRef.current.exportImage("png");
    localStorage.setItem("remnantSheetCanvas", remnantCanvasData);
    alert("Shape Saved Successfully. You can close this tab now.");
    setTimeout(() => (window.location.href = "/addremnantstocks"), 1000);
  };

  return (
    <div className="relative">
      <div>Remnant Canvas</div>
      <ReactSketchCanvas
        ref={canvasRef}
        width={width}
        height={height}
